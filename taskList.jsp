...
<%	
	...
	try {
		...
		new LogEntity( LogEntity.TIPO_SITE, usuarioLogado.getIdUsuario(), usuarioLogado.getTableName(), usuarioLogado.get_IDFieldName(), request.getRemoteAddr(), request.getLocalName(), 0l, "Reunião: lista");
...
<div class="clear20"></div>
	<div>	
		...
		<div class="inline">			
			<input type="text" id="filtroDescricao" class="filteradminText input-medium" name="filtroDescricao" placeHolder="Descrição" value="<%= filtroDescricao %>"/>
		</div>
		<div class="inline">			
		...
		<%	if( !usuarioLogado.semPermissaoAcesso(idFunc, "incluir") ) { %>
			<div class="inline">
				<button class="btn btn-primary btn-circle grouped_elements" href="calendarioEdit.jsp" style="width: 200px"><span class="iconfa-plus-sign icon-white"></span> <%= p.get("ADICIONAR") %> Reunião</button>
			</div>
		<%	} %>
		<div>
	...
	<!--<table class="table table-bordered responsive" id="admin"> -->
		...
		<table class="table table-bordered responsive" id="admin">
			...
			<thead>
				<tr>								
					<th class="head1 headerSortDown sortable">DESCRIÇÃO</th>	
					<th class="head2 headerSortDown sortable">SALA</th>				
					<th class="head3 headerSortDown sortable">DATA INÍCIO</th>
					<th class="head4 headerSortDown sortable">DATA FIM</th>				
					<th class="head5 nosortable">AÇÕES</th>
				</tr>
			</thead>
			<tbody>
			</tbody>
...
$(document).ready(function(){
	...
	$(document).on("click",".btn-excluirReuniao",function(e){
		...
		confirmBox("Excluir", "Deseja excluir a reunião?", function(x){
    		...
    		if(x){
    			...
    			.fail(function(error){
    				...
    				warningAlert("Mensagem","Erro ao Excluir a reunião",function(){});
