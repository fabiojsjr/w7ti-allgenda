	...
	public boolean retrievePass(){
		try {
			if(getThis("email='"+getEmail()+"'")){
				SendMailSparkpost.send("Confirmaééo de Cadastro", "sender@tetrasoft.com.br", getEmail(), "text/html", new StringBuffer("<table><tr><td>"+getSenha()+"</td></tr></table>") );
			}
		} catch (Exception e) {
			return false;
		}
		...
		try {
			...
			if( checkLogin.getThis("login = '" + this.getLogin() + "'",conn) ) {				
				if( !checkLogin.getIdUsuario().toString().equals( this.getIdUsuario().toString() ) ) {
					throw new Exception("Login jé existe, por favor, selecione outro!");
				}
			}
			if( checkLogin.getThis("email = '" + this.getEmail() + "'",conn) ) {
				if( !checkLogin.getIdUsuario().toString().equals( this.getIdUsuario().toString() ) ) {
					throw new Exception("E-Mail jé existe, por favor, selecione outro!");
				}
			}
