...
//    	"Suporte Técnico",
	...
	public void save(String email, String tel, String msg, String assunto, String nome, String anexo) throws Exception {		
		try {
			...
			if(checkOS.getThis(query + " and assunto = " + assunto+" order by dataMensagem desc")&& checkOS.getAssunto().equals(assunto) && checkOS.getResposta() == null){
				throw new Exception("Aguarde a Resposta de seu último chamado.! ");
			}else{
			...
			conteudo += "		Você abriu um chamado no suporte:<br/><br/>";
			conteudo += "		Protocolo: " + this.getIdSuporte() + "<br/><br/>";
			conteudo += "		Usuário: " + user.getNome() + "<br/><br/>";
			...
			SendMailSparkpost.send("Suporte Técnico", ConfigEntity.CONFIG.get("email"), email , "text/html", new StringBuffer(conteudo) );
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void responder(String id, String resposta, String assunto, String idUsuarioResposta, String anexo, String tipoResposta, String respostaAntiga) throws Exception {
		try {
			...
			conteudo += "		Olá, " + ((user.getNome() != null && !user.getEmail().equals("")) ? user.getNome() : os.getNome()) + ", você enviou a seguinte dúvida abaixo para nossa equipe de atendimento: " + os.getMensagem() + " <br/><hr/>";
			...
			conteudo += "		Qualquer dúvida, estamos à disposição.";
			conteudo += "		<br/>";
			...
			if(tipoResposta.equals("responder")){
				SendMailSparkpost.send("Suporte Técnico", ConfigEntity.CONFIG.get("email"), (user.getEmail()!=null && !user.getEmail().equals("")) ? user.getEmail() : os.getEmail(), "text/html", new StringBuffer(conteudo) );
			}else{
				SendMailSparkpost.send("Fechamento do Chamado - Suporte Técnico", ConfigEntity.CONFIG.get("email"), (user.getEmail()!=null && !user.getEmail().equals("")) ? user.getEmail() : os.getEmail(), "text/html", new StringBuffer(conteudo) );
			}
