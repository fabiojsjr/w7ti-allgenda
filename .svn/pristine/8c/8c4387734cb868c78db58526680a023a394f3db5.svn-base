...
<%
	...
	try {		
			...
			try{
				...
				if(idReuniao != null){
					...
					if(task2.getIdTask() == null){
						out.println("<input type='hidden' name='msgErroExcluir' value='Reunião não encontrada'/>");
						throw new ExceptionWarning("Reunião não encontrada");	
					}
		...
		//verifica se a sala está com o horario disponivel
		...
		if( request.getParameter("insert") != null ) {
			...
			new LogEntity( LogEntity.TIPO_SITE, usuarioLogado.getIdUsuario(), task.getTableName(), task.get_IDFieldName(), request.getRemoteAddr(), request.getLocalName(), new Long(idTask), "Edição de calendário (save)", conn);
		...
		new LogEntity( LogEntity.TIPO_SITE, usuarioLogado.getIdUsuario(), task.getTableName(), task.get_IDFieldName(), request.getRemoteAddr(), request.getLocalName(), new Long(id), "Edição de calendário", conn);
%>	
		...
		<div class="span7" style="padding: 0px; margin: 0px; width: 100%">
			...
			<!-- 	<p class="control-group-int">
					...
					<p class="control-group-int">
						...
						<textarea type="text" name="nomesParticipantes" placeholder="Separe os nomes com ponto e vírgula" style="max-width : 290px;"><%= task.getNomesParticipantes() == null ? "" : task.getNomesParticipantes() %></textarea>		
					</p>
				<!-- <p class="control-group-int">
						<label class="control-label">E-mails: </label>
						<textarea type="text" name="emails" placeholder="Separe os e-mails com ponto e vírgula" style="max-width : 290px;"><%= task.getEmails() == null ? "" : task.getEmails() %></textarea>		
					</p> -->
					<p class="control-group-int">
...
<script>
	...
	$(document).ready(function() {	
		...
		//seta a label do input de duração ao abrir a tela
		...
		//seta o valor da duração da reunião ao editar
	...
	//faz validações e envia o formulario
	...
	function sendForm(){		
		...
		$.get('calendarioEdit.jsp?verificaParticipantes=true&idSalaAtual=' + $("[name='salas']").val() + "&participantes=" + $("[name='participantes']").val(),function(data){
			...
			if(mensagemErro != null || mensagemErro != undefined){				
				...
				//essa função está amarrada com o calendário
				//esse if serve para não da Erro na tela de taskList ao editar a reuniao
				if($(".pagetitle > h1").text() != "Reuniões")
...
$("#updateTaskButton").click(function(e){
	...
	$(".btnExcluirTask").click(function(){
		...
		$.get("calendarioEdit.jsp?ExcluirReuniao=true&id=" + idReuniao,function(data){
			...
			if(mensagemErro != null || mensagemErro != undefined){				
				...
				calendario(1, <%= idCalendario == null ? "0" : idCalendario %>);
				infoAlert("Mensagem","Reunião excluída com sucesso",function(){});					
				setTimeout(function(){
	...
	$(document.formTask).validate({
		...
		messages :{
			...
			duracaoReuniao: "Escolha a duração",
			...
			if(valorDuracao == 0){
				if($(".container-duracao").children().length <= 0){
					$(".container-duracao").prepend(
							"<label class='erroMsg'>Escolha a duração</label>"		
						);	
				}
