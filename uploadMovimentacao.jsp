...
function beforeUpload(){
	...
	var response = false;
		...
		if(arquivo.val().indexOf(".") < 0){
			window.parent.warningAlert("Mensagem", "Erro, Arquivo inválido" , function(){});
		}else if(!validateExtension(extension)){
			window.parent.warningAlert("Mensagem", "Erro, Arquivos do tipo: "+getNomeExtension(extension)+" não séo permitidos.", function(){});
		}else if(response){
			window.parent.warningAlert("Mensagem", "Erro, Este arquivo jé foi enviado." , function(){
			...
			try{
				...
				file = arquivo[0].files[0];
					if(file.size == 0 ){
						window.parent.warningAlert("Mensagem", "Erro, tamanho do arquivo inválido" , function(){});
						return;						
					}
				}
			}catch(err){
